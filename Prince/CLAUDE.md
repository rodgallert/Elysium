...
- Known pending cleanup: `Microsoft.AspNetCore.OpenApi` 2.0.0 (in all three services' templates) has a known high-severity NuGet advisory (GHSA-v5pm-xwqc-g5wc) — bump it opportunistically or when asked. Also: `Prince.Checkout.Api` builds with an `MSB3277` NuGet version-conflict warning between two `Microsoft.EntityFrameworkCore` versions pulled in transitively via `Prince.Data` — doesn't fail the build, but Checkout has no direct EF Core package reference of its own to pin a consistent version, unlike `Prince.App` which needed one for the same reason.
