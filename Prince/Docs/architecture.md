...
`Producer`, `Product`, `Offer`, `Transaction`, and `Withdrawal` all now inherit a shared `BaseEntity` (`Models/Shared/BaseEntity.cs`, just `Id`) rather than each declaring their own. `Prince.Domain/Interfaces/Repository/IRepository.cs` is the first repository contract (`GetAsync`/`ListAsync`/`AddAsync`/`UpdateAsync`/`DeleteAsync`, constrained to `BaseEntity`) — entity-specific repository interfaces and any concrete EF Core implementation are still ahead.

**Persistence**: `Prince.Data`/`Prince.App` are wired up for real now — the four Core projects' references were connected for the first time (`Prince.App → Prince.Data → Prince.Domain`), `PrinceDbContext` maps all five entities (code-first, `EFCore.NamingConventions` for snake_case), and migrations run automatically on every `core-api` startup (`db.Database.MigrateAsync()`, idempotent — a no-op when already up to date). `Prince.Services` is still an empty stub, not yet wired into `Prince.App`. See `Docs/decisions.md` for two real bugs caught during this work (a migrations-history-table collision between `PrinceDbContext` and Jobs' `JobsIdentityDbContext` sharing one physical database, fixed by giving each its own history table name) and how the identity-corruption risk was ultimately resolved: **`Id` is now database-generated** (`gen_random_uuid()` Postgres column default, configured once for every `BaseEntity`-derived type), not client-generated — the earlier private-constructor workaround addressed the symptom, this removes the cause (no constructor assigns `Id` at all anymore). Verified empirically: logged an entity's `Id` before/after `SaveChangesAsync`, confirmed the generated `INSERT` omits the `id` column and reads the Postgres-generated value back via `RETURNING id`.
